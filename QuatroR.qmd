...
for (txt_file in txt_files) {
  ...
  possible_separators <- c("\t", ";")  # Add other separators if needed
...
new_data <- read_excel("./zonesdarrets.xlsx")  # Replace with the actual path
new_data
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("update_btn", "Update Comparison"),
...
# Create a leaflet map for station selection
  output$map_station <- renderLeaflet({
    leaflet() %>%
      ...
      addMarkers(data = final_data_sf, lng = st_coordinates(final_data_sf$geometry)[,"X"], lat = st_coordinates(final_data_sf$geometry)[,"Y"], popup = ~ZdCId, layerId = ~ZdCId)
  })
...
-   For the deployment we used shinyapps.io but the problem was that the free plan of this website provides only 1G of RAM as maximum. even after reducing the data size and using only 1 semester as a solution the Out Of Memory problem still occured because the data and the tasks done requires a lot more memory. The app was run locally but on a 24G RAM PC.

-   Here are some screenshots that the deployment is done :

![](images/clipboard-3429202612.png)

![](images/clipboard-675126278.png)

OOM problem:

![](images/clipboard-866994325.png)

![](images/clipboard-1490589384.png)
